...
export default function Hero() {
  return (
    <section className="relative bg-gradient-to-r from-blue-600 to-purple-600 text-white min-h-screen flex flex-col items-center justify-center py-16">
      <div className="max-w-6xl text-center px-6">
        ...
        {Object.entries(imageCollections).map(([title, images]) => (
          <ImageGrid key={title} title={title} images={images} />
        ))}
      </div>
    </section>
  );
...
function ImageGrid({ title, images }: ImageGridProps) {
  return (
    <div className="mb-16">
